...
#------------------------------------------------------------------------------
    ...
    for t in range(len(bars)):
        # 条件判断
        longs = df['dayopen'][t] <= df['low'][t-1]*(1-entryZscore2*movingstd[t])
        shorts = df['dayopen'][t] >= df['high'][t-1]*(1+entryZscore1*movingstd[t])
...
if __name__ == '__main__':
    ...
    pf, Accountsummny = runstrategy('../../ts_data/day/M.DCE.addopen.csv',0.1,0.1,90)
