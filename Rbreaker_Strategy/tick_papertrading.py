...
"""先读取昨日数据"""
ldata = w.wsd("USDJPY.FX", "open,high,low,close,volume,amt", "2016-11-04", "2016-11-04", "")
...
noontime = datetime.strptime('23:00',"%H:%M").time()
ExTime = datetime.strptime('23:45',"%H:%M").time()
flag2 = 0
...
def runstrategy(bar):
    ...
    if StartTime <= bar['time'] < ExTime and shouldopen:
        ...
        if revershorts and bar['bid1'] != 0 and bar['time'] <= noontime and flag2 == 0:
            ...
            if Pos[-1] == 1:
                ...
                winsound.PlaySound('closepos.wav', winsound.SND_ASYNC)
                winsound.PlaySound('openpos.wav', winsound.SND_ASYNC)
            ## 开仓做空
        ...
        #openposprice = Openorder[:t][Openorder[:t] != 0][-1]['Openpos']
            if bar['low'] <= Openorder[-1]['Openpos']*(1-stoploss):
                ...
                Closeorder.append({'Type': -1,'Closepos':entryprice,'Vol':1,'Time': bar['time']})
                print ("Closeorder:" , Closeorder[-1])
                winsound.PlaySound('stoploss.wav', winsound.SND_ASYNC)
                # 加个止损标志，以防止今天再次开仓
                flag2 = 1
        if Pos[-1] == -1:
            #openposprice = Openorder[:t][Openorder[:t] != 0][-1]['Openpos']
    ...
    if bar['time'] >= ExTime:
        ...
        if Pos[-1] == 1:
            ...
            Closeorder.append({'Type': -1,'Closepos':entryprice,'Vol':1,'Time': bar['time']})
            print ("Closeorder:" , Closeorder[-1])
            winsound.PlaySound('closetime.wav', winsound.SND_ASYNC)
        if Pos[-1] == -1:
            Pos.append(0)
            entryprice = bar['open']
            Closeorder.append({'Type': 1,'Closepos':entryprice,'Vol':1,'Time': bar['time']})
