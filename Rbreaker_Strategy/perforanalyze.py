...
def MaxDD(cumret):
    ...
    ## 240mins
    maxDDD = int(max(drawdownduration)/240) # maximum drawdown duration
    #drawdownList = drawdown
    return maxDD, maxDDD, drawdown

###-----------------------------------------------------------------------------
"""--收益分析--"""
def stratanalyz(Accountsummny,TradingCost=0.3/10000,save = False):
    ### 转换时间戳,
    ##  TODO: 判断时间格式
    '''
    ...
    ### TOTO:判断时间周期，252需要改
    fre = 252*4*60
    APR = np.prod(1+ret)**(float(fre)/len(ret))-1
    Avg_Ann_Ret = fre*ret.mean()
    Ann_Volatility = np.sqrt(fre)*ret.std()
    Sharpe_ratio = np.sqrt(fre)*ret.mean()/ret.std()
...
def ploter(Accountsummny, save=False):
    ...
    if len(ret.index.values[0]) >= 12:
        tdate = map(lambda x:datetime.strptime(x, '%Y/%m/%d %H:%M').date(),
                    ret.index.values)
    else:
